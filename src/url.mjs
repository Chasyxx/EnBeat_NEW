...
export function getCodeFromUrl(hash) {
	...
	if(hash.startsWith('#4')) {
		...
		try {
			songData = {
				mode: ['Bytebeat', 'Signed Bytebeat', 'Floatbeat', 'Funcbeat', 'Bitbeat', '2048', 'logmode', 'loghack', 'loghack2'][dataArr[0]],
...
export function getUrlFromCode(code, mode, sampleRate) {
	...
	outputArr[0] = ['Bytebeat', 'Signed Bytebeat', 'Floatbeat', 'Funcbeat', 'Bitbeat', '2048', 'logmode', 'loghack', 'loghack2'].indexOf(mode);
